...
import { useCmsListLeads, useCmsUpdateLeadStatus, useCmsDeleteLead, cmsExportLeads } from "@workspace/api-client-react";
...
export default function AdminRegistrations() {
  ...
  const handleExport = async () => {
    setIsExporting(true);
    try {
      const result = await cmsExportLeads();
      if (result) {
        ...
        toast({ title: "Xuất file thành công" });
      }
    } catch {
    ...
    <AdminLayout>
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="flex gap-3">
          ...
          <Button onClick={handleExport} variant="outline" className="rounded-none border-primary text-primary">
      ...
      <div className="grid grid-cols-5 gap-3 mb-6">
        {Object.entries(STATUS_LABELS).map(([status, label]) => {
          const count = data?.registrations?.filter(r => r.currentStatus === status).length || 0;
          return (
            <button
              key={status}
        ...
        <Table>
          ...
          <TableBody>
            ...
            {!isLoading && (!data?.registrations || data.registrations.length === 0) && (
              <TableRow><TableCell colSpan={8} className="text-center py-12 text-gray-500">Chưa có dữ liệu</TableCell></TableRow>
            )}
          </TableBody>
        </Table>
      </div>
